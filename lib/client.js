...
/**
 * Finds repos by query on meta
 * @param query
 * @returns {Promise}
 */
GitClient.find = function (query) {

	return new Promise(function (resolve, reject) {
		GitClient._mongo.collection(COLLECTION_REPOS).find(query).toArray(function (error, list) {
			/* istanbul ignore next */
			if (error) return reject(error);
			var result = [];
			list.forEach(function (repo) {
				result.push(new (require('./file'))(repo));
			});
			resolve(result);
		});
	});
